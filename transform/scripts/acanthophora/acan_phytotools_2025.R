...
# 1) Open concatenated and cleaned data
acan_pt <- read.csv("/Users/angela/src/Photosynthesis/data/acan_2024/transformed/acan_2024_pamdata_clean_tidy.csv", sep = ",")
# 1a) Get rid of junk rows
acan_pt <- acan_pt %>%
  filter(!id %in% c("test1", "test2", "Ac108 do not use"))

# 1b) Add values for nitrate and salinity
acan_pt <- acan_pt %>%
  mutate(
    nitrate  = if_else(as.integer(str_sub(id, -1)) %% 2 == 1, 0.5, 14),
    salinity = if_else(as.integer(str_sub(id, -1)) %% 2 == 1, 35, 28)
  )
# 1c) Add temperature column
acan_pt <- acan_pt %>%
  mutate(id_number = as.integer(str_sub(id, -2)))  # Extract numeric part from ID  

acan_pt <- acan_pt %>%
  mutate(
    temp = if_else(((id_number - 1) %/% 8) %% 2 == 0, 28, 24)
  )

# 1d) Make sure date is in Date format
acan_pt <- acan_pt %>%
  mutate(date = ymd(date)) %>%
  mutate(time = as_hms(time))  # Ensure time is in hms format)

# 1e) Add RLC day
acan_pt <- acan_pt %>%
  mutate(rlc_day = case_when(
    date == as.Date("2023-11-16") ~ 1,
    date == as.Date("2023-11-20") ~ 5,
    date == as.Date("2023-11-24") ~ 9,
    date == as.Date("2023-12-04") ~ 1,
    date == as.Date("2023-12-08") ~ 5,
    date == as.Date("2023-12-12") ~ 9,
    TRUE ~ NA_real_
  ))
# Add unique_id for all individuals
acan_pt <- acan_pt %>%
...
first_row_of_rlc <- acan_pt %>%
  filter(epar == 0 & is.na(npq))
last_row_rlc <- subset(acan_pt, delta_npq > 0)
...
#write.csv(pmax_ek_alpha, "/Users/angela/src/Photosynthesis/data/acan_2024/transformed/acan2024_ek_alpha_normalized.csv")
